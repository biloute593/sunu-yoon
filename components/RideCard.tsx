...
const RideCard: React.FC<RideCardProps> = ({ ride, onClick, onBook, onContact }) => {
  const departureDate = new Date(ride.departureTime);
  const timeString = departureDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
  
  // Si le format de date est invalide, afficher un fallback
  const displayTime = timeString === 'Invalid Date' ? '--:--' : timeString;
    ...
    >
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="text-right">
          <span className="block text-2xl font-bold text-emerald-600">
            {ride.price?.toLocaleString('fr-FR') || '0'}
          </span>
          <span className="text-xs text-gray-400">{ride.currency || 'XOF'}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-gray-100 mt-auto">
        <div className="flex items-center gap-4">
          <div className="relative">
            <img
              src={ride.driver.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(ride.driver.name)}&background=10b981&color=fff`}
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500">
              ...
              <span className="font-medium text-gray-700">{ride.driver.rating || '4.5'}</span>
              <span className="text-gray-400">({ride.driver.reviewCount || '0'})</span>
            </div>
          </div>
        </div>
