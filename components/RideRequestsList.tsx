  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const msg = data?.error?.message || data?.error || data?.message || 'Erreur de chargement';
        throw new Error(msg);
      }
    } catch (err: any) {
  ...
  const handleAccept = async (requestId: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const msg = data?.error?.message || data?.error || data?.message || 'Erreur lors de l\'acceptation';
        throw new Error(msg);
      }
    } catch (err: any) {
      alert(err.message);
