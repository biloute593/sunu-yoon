...
const ProfileView: React.FC<{
  ...
  const [myRides, setMyRides] = useState<FrontendRide[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMyRides(rides.map(mapApiRideToFrontend));
  ...
  const handleConfirmBooking = async (bookingId: string) => {
    try {
      ...
      await loadData();
      alert('Réservation confirmée ! Le passager sera notifié.');
    } catch (error) {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 mb-8 flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
            ...
            <button onClick={logout} className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-sm font-medium transition-colors">
              Déconnexion
            </button>
          </div>
        </div>
            ...
            driverRequests.map(req => (
              <div key={req.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="flex gap-3">
                  ...
                  <a href={`tel:${req.passenger.phone}`} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors">
                    <Icons.Phone size={18} />
                  </a>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {booking.status === 'confirmed' && booking.ride.driver.phone && (
                      <div className="flex gap-2">
                        <a href={`tel:${booking.ride.driver.phone}`} className="p-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors" title="Appeler">
                          <Icons.Phone size={20} />
                        </a>
                        <a
