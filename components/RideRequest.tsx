...
const RideRequest: React.FC<RideRequestProps> = ({ userLocation, onRequestRide, activeRequest = null, onCancelRequest }) => {
  ...
  useEffect(() => {
    ...
    if (activeRequest.status === 'accepted' && activeRequest.driver) {
      setMatchedDriver({
        ...
        carModel: activeRequest.driver.carModel || 'Véhicule',
        isAvailable: true
      });
  ...
  const renderSearching = () => (
    <div className="bg-white rounded-3xl shadow-xl p-8 text-center">
      ...
      <p className="text-gray-600 mb-6">Votre demande a été envoyée aux chauffeurs disponibles à proximité.</p>
      ...
      <div className="mt-6 bg-emerald-50 border border-emerald-200 rounded-2xl p-5 text-left space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500">Départ: {pickupAddress || '—'} • Arrivée: {dropoffAddress || '—'}</p>
          </div>
        ...
        <p className="text-sm text-gray-600">
          Dès qu'un chauffeur accepte, vous verrez son nom, son véhicule et son délai d'arrivée ici.
        </p>
      </div>
    </div>
  );
  ...
  const renderMatched = () => (
    <div className="bg-white rounded-3xl shadow-xl p-6">
      ...
      <div className="space-y-3">
        ...
        <button
          onClick={() => {
            if (activeRequest?.id && onCancelRequest) {
