    ...
    <div className="min-h-screen flex flex-col bg-gray-50 font-sans text-gray-900">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center space-x-4">
              ...
              <button 
                onClick={() => onNavigate('ride-request')}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-full transition-all bg-rose-50 text-rose-700 hover:bg-rose-100"
              >
                🚖 Course immédiate
              </button>
              <button 
          ...
          <div className="md:hidden bg-white border-t border-gray-100 absolute w-full z-50 animate-fade-in-down shadow-xl">
            <div className="px-4 pt-2 pb-4 space-y-1">
              ...
              >
               ...
               <button 
                 onClick={() => { onNavigate('ride-request'); setIsMenuOpen(false); }}
                 className="flex items-center gap-3 w-full text-left px-4 py-3 rounded-lg text-base font-medium text-rose-700 hover:bg-rose-50"
               >
                 <Icons.Navigation size={20} />
                 Course immédiate
               </button>
               <button 
                 onClick={() => { onNavigate('driver-mode'); setIsMenuOpen(false); }}
                 className="flex items-center gap-3 w-full text-left px-4 py-3 rounded-lg text-base font-medium text-yellow-700 hover:bg-yellow-50"
               >
                 <Icons.Car size={20} />
                 Mode Chauffeur
               </button>
      ...
      <footer className="bg-gray-900 text-white mt-auto">
        <div className="max-w-6xl mx-auto px-4 pt-12 pb-8 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="text-gray-400 text-sm leading-relaxed mb-4">
                La plateforme de covoiturage de confiance au Sénégal. Voyagez ensemble, économisez mieux.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm text-gray-400">
                ...
                <li><button onClick={() => onNavigate('ride-request')} className="hover:text-emerald-400 transition-colors text-left">Course immédiate</button></li>
            ...
            {/* Support */}
            <div>
              <h4 className="text-xs font-semibold text-gray-300 uppercase tracking-wider mb-4">Aide & Support</h4>
              <ul className="space-y-2.5 text-sm text-gray-400">
                <li><span className="hover:text-emerald-400 transition-colors cursor-pointer">Comment ça marche ?</span></li>
                <li><span className="hover:text-emerald-400 transition-colors cursor-pointer">Questions fréquentes</span></li>
                <li><span className="hover:text-emerald-400 transition-colors cursor-pointer">Signaler un problème</span></li>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <a href="#" aria-label="Twitter/X" className="w-8 h-8 bg-gray-800 hover:bg-emerald-600 rounded-full flex items-center justify-center transition-colors text-gray-400 hover:text-white text-xs font-bold">x</a>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-800 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3">
            <p className="text-gray-500 text-sm">© 2025 Sunu Yoon Sénégal. Tous droits réservés.</p>
            <div className="flex gap-4 text-xs text-gray-600">
              <span className="hover:text-gray-400 transition-colors cursor-pointer">Confidentialité</span>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 z-40" role="navigation" aria-label="Navigation mobile">
        <div className="flex items-end justify-around px-2 pt-1 pb-2">
          <button
            onClick={() => onNavigate('home')}
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl transition-colors ${currentView === 'home' ? 'text-emerald-600' : 'text-gray-400 hover:text-gray-600'}`}
            aria-label="Accueil"
          >
            <Icons.Car size={22} />
          ...
          <button
            onClick={() => onNavigate('search')}
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl transition-colors ${currentView === 'search' || currentView === 'ride-details' ? 'text-emerald-600' : 'text-gray-400 hover:text-gray-600'}`}
