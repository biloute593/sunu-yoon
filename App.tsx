...
import { Coordinates, LocationState, DraftRide, Ride, SearchParams } from './types';
...
// ErrorBoundary class remains here for now
...
class ErrorBoundary extends React.Component<{ children: React.ReactNode, name?: string }, { hasError: boolean, error: any }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-8 bg-red-50 border-2 border-red-200 rounded-2xl m-4 overflow-auto max-h-[80vh]">
          <h2 className="text-xl font-bold text-red-700 mb-2">Oups ! Une erreur est survenue dans {this.props.name || 'l\'application'}</h2>
          <p className="text-red-600 font-bold mb-2">{this.state.error?.message || 'Erreur inconnue'}</p>
          <pre className="text-xs text-red-500 bg-red-100 p-4 rounded-lg whitespace-pre-wrap mb-4">
            {this.state.error?.stack || 'Pas de stack trace disponible'}
          </pre>
          <button 
...
function AppContent() {
  ...
  const handlePublishRide = async (createdRide: ApiRide) => {
    console.log('Trajet publié, mise à jour des données en arrière-plan...');

    // NE PAS naviguer immédiatement — laisser PublishForm afficher l'écran de succès
    // La navigation sera gérée par onCancel dans PublishForm après que l'utilisateur a vu le succès

    // Mettre à jour les données en arrière-plan pour que l'accueil soit frais quand on y revient
    try {
    ...
    // Refresh: recharger depuis l'endpoint /recent (sans changer de vue)
    try {
